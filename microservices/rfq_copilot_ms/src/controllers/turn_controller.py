...
Batch 4 flow:
- general mode    -> canned reply (no LLM, no manager call)
- rfq_bound mode  -> RfqGroundedReplyService (manager fetch + LLM)
...
class TurnController:
    ...
    def process_turn(
        ...
        history = all_turns[:-1]  # drop the just-appended user_row
        ...
        if thread.mode_kind == "rfq_bound" and thread.rfq_id:
            ...
            # general mode (or rfq_bound with missing rfq_id — defensive)
