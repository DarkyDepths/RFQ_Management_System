...
class V2TurnResponse(BaseModel):
    ...
    * Other status values are returned via ``JSONResponse`` with the
    ...
    """Per-turn UUID generated by the controller. Optional for
    forward-compat with future entry shapes; populated for every
    successful turn in Slice 1."""
    ...
    """The resolved RFQ code (if any) — convenience field for the
    frontend so it can correlate the answer to a card without
    re-parsing the answer body. None for Path 1 / Path 8.x or any
    turn where no target was resolved."""
